...
function Strategy(options, verify) {
  ...
  if (!verify) { throw new TypeError('AuthbusStrategy requires a verify function') }
  passport.Strategy.call(this);
  this.name = 'authbus';
  this._verify = (req, token, verified) => {
    let authbusEndpoint = options.authbusEndpoint;
    ...
    Promise.resolve(authbusEndpoint).then(endpoint => {
      return axios({
        ...
        verify(user).then(_user => verified(null, _user));
      }).catch(() => {
          verified(null, false);
      });
    });
