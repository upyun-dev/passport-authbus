...
function Strategy(options, verify) {
  ...
  if (!options.authbusEndpoint) { throw new TypeError('AuthbusStrategy requires authbusEndpoint option'); }
  if (!verify) { throw new TypeError('AuthbusStrategy requires a verify callback'); }
  
  passport.Strategy.call(this);
  this.name = options.name || 'authbus';
  this._verify = verify;
  this._realm = options.realm || 'Users';
  if (options.scope) {
    this._scope = (Array.isArray(options.scope)) ? options.scope : [ options.scope ];
  }
  this._authbusEndpoint = options.authbusEndpoint;
...
Strategy.prototype.authenticate = function(req) {
  ...
  let authbusEndpoint = this._authbusEndpoint;
  if (typeof authbusEndpoint === 'function') {
    authbusEndpoint = authbusEndpoint(req);
  }
 
  Promise.resolve(authbusEndpoint).then(endpoint => {
    if (!endpoint) {
      return this.error(new TypeError('Invalid authbusEndpoint'));
    }
    ...
    return axios({
      ...
      this.error(createError(status, data));
    });
  });
